 ...
 *
 *	Details:
 *		For more information about the functions in this file refer to
 *<th_lib.c>
 */
/*------------------------------------------------------------------------------
...
#if _MSC_VER >= 1500
/* visual studio 9 and above needs all these before th_types.h to avoid
 * conflicts */
/* Note : this is only for a non embedded system. Please test with gcc before
 * comitting anything resolved with VS9 */
#include <sal.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#else
/* if BMDEBUG off, make this a local useless function to allow compiler to
 * easily optimize it away */
static inline int th_dprintf(const char *fmt, ...) {
  if (fmt)
    return 1;
  else
    return 0;
...
#define th_aligned_malloc(size, align)                                         \
  th_aligned_malloc_x(size, align, __FILE__, __LINE__)
void *th_aligned_malloc_x(size_t size, size_t align, const char *file,
                          int line);
...
/* Endianess Stuff */
#define ee_32_swap(_x)                                                         \
  ((((_x) & 0x000000FF) << 24) | (((_x) & 0x0000FF00) << 8) |                  \
   (((_x) & 0x00FF0000) >> 8) | (((_x) & 0xFF000000) >> 24))
...
void al_main(
    int argc,
    char *argv[]); /* this function is in th_al.c, but accessed directly */
...
int th_send_buf_as_file(const unsigned char *buf, size_t length,
                        const char *fn);
...
/* Function: th_randseed
        Return a seed that can be used to recreate random sequence from current
   state.
*/
...
#if FP_KERNELS_SUPPORT
#define ee_snr(sig, ref)                                                       \
  ((e_f64)(sig) / ((e_f64)(sig) - (e_f64)(ref))) *                             \
      ((e_f64)(sig) / ((e_f64)(sig) - (e_f64)(ref)))
...
functions. No implementations due to some compilers not supporting vsscanf.
...
/* Function: th_parse_flag
        parse argv looking for an int value from a flag, in the form of
   <flag><value>. */
#if USE_CTYPE
#include <ctype.h>
...
/* Function: th_parse_buf_flag
        parse a string looking for an int value from a flag, in the form of
   <flag><value>.
*/
...
/* Function: th_parse_buf_fpflag
        parse a string looking for an floating point value from a flag, in the
   form of <flag><value>.
*/
...
/* Function: th_get_flag
        parse argv looking for a string flag from a flag, in the form of
   <flag><value>.
*/
int th_get_flag(int argc, char *argv[], char *flag, char **val);
/* Function: th_get_buf_flag
        parse a string looking for a string flag from a flag, in the form of
   <flag>...
*/
int th_get_buf_flag(char *buf, char *flag, char **val);
/* Function: th_dup_buf_flag
        Parse a string looking for a string flag from a flag, in the form of
   <flag>..., then strdup the string.
        * When using this function, the memory returned in *val needs to be
