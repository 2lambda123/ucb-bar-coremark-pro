...
/*************************
** Loops Tests **
** Perform a variety of small loops tests, taken from scimark and livermore
*loops.
*************************/
#include "loops.h"
#include "th_cfg.h"
...
/* ======================================================================== */
/* file provides :
define - init base input params (segment [within 0..2 boundary], number of
coefficients to calculate, number of integration steps) init -  allocate working
memory, assign a[0] and b[0] . run - calculate N coefficients in the segment
fini - calculate avg of coeffients then dealloc working memory
verify - SNR average of coefficients vs golden reference, must run at least base
iterations for valid output. clean - deallocate output memory
*/
...
benchfunc all_tests[] = {MonteCarlo_integrate,
                         hydro_fragment,
                         cholesky,
                         inner_product,
                         banded_linear,
                         ...
                         NULL};
#if VERBOSE
char *test_type_str[] = {"MONTE_CARLO_PI_IDX			",
                         "HYDRO_IDX					",
                         "CHOLESKY_IDX				",
                         "INNER_PRODUCT_IDX			",
                         "BANDED_LINEAR_IDX			",
...
uint32_t hex(float f) { return *((uint32_t *)(&f)); }
...
void reinit_ivec_sparse(loops_params *params, e_u32 *p, int nvals, e_u32 mask,
                        int step) {
  int i = 0;
  int si = random_u32(params->r);
  while (i < nvals) {
...
void *define_params_loops(unsigned int idx, char *name, char *dataset) {
  ...
  default_disable_mask =
      ~(FIRST_SUM | FIRST_DIF | FIRSTMIN | TRI_DIAGONAL | LINEAR_RECURRENCE |
        IMPLICIT | HYDRO_2D | INNER_PRODUCT);
  ...
  /* generic inits */
  {
    ...
    switch (params->rtype) {
    case 3:
      r = rand_intparts_init(params->seed, 256, 1, -1, 7, 0,
                             EE_LIMIT_DYNAMIC_RANGE(0x00ffffff));
      params->rbank =
          fromint_random_fp_vector(0x1000, r); /* random bank -1..1 */
      break;
    case 2:
      params->rbank = fromint_fp_01_vector_uniform(
          0x1000, params->seed); /* random bank 0..1 */
      break;
    case 1:
      r = rand_intparts_init(params->seed, 256, 1, 0, 0, 0,
                             EE_LIMIT_DYNAMIC_RANGE(0x0000ffff));
      params->rbank =
          fromint_random_fp_vector(0x1000, r); /* random bank, random sign */
      break;
    default:
      r = rand_intparts_init(params->seed, 256, 0, 0, 0, 0,
                             EE_LIMIT_DYNAMIC_RANGE(0x0000ffff));
      params->rbank = fromint_random_fp_vector(
          0x1000, r); /* random bank with reduced probability of denormals */
      break;
    }
    for (i = 0; i < 0x1000; i++) {
      if (i & 1)
        params->rbank[i] -= FPCONST(0.99);
      else
        params->rbank[i] -= FPCONST(1.01);
    }
...
void *bmark_init_loops(void *in_params) {
  ...
  for (i = 0; i < num_vectors; i++) {
    ...
    if (i < 4)
      ...
      spacer = 0;
    myparams->v[i] =
        (e_fp *)th_malloc(sizeof(e_fp) * (myparams->vsize + spacer));
...
int bmark_verify_loops(void *in_params) {
  ...
  if (params->gen_ref) {
    ...
    if (params->ref_data == NULL) {
      th_printf("Cannot validate this dataset! Please generate reference data "
                "on reliable hardware and tools using -g!\n");
      return 0;
    }
...
e_fp hydro_fragment(loops_params *p) {
   ...
   *
   * 	SG: Original code had only the last iteration of the outer loop
   *mattered. modified to get feedback from x between iterations.
   */

  for (l = 1; l <= loop; l++) {
    reinit_vec(p, z, n + 11); /* force multiple iterations with new data for
                                 each iteration */
    for (k = 0; k < n; k++) {
...
e_fp inner_product(loops_params *p) {
   ...
   *  3      Q= Q + Z(k)*X(k)
   *
   *	SG : original code only had to perform one loop. Moved q initialization
   *out of loop.
   */
...
e_fp banded_linear(loops_params *p) {
  e_fp *x = p->m2[0]; /* really a vector, but needs size N+1001, so use matrix
                         memory instead */
  e_fp *y = p->v[1];
  int n = p->N, loop = p->Loop;
  int m, l, lw, j, k;
  ...
  if (p->N < 1001) {
    th_exit(THE_FAILURE,
            "Bad size for banded linear equations, must be at least 1001");
  }
  /*
   *******************************************************************
...
e_fp tri_diagonal(loops_params *p) {
   ...
   *  5    X(i)= Z(i)*(Y(i) - X(i-1))
   *
   *	SG: Original code only had to execute one loop. Modified with carry over
   *to first index.
   */
  ...
  for (l = 1; l <= loop; l++) {
    for (i = 1; i < n; i++) {
      x[i] = z[i] * (y[i] - x[i - 1]);
      if (!th_isfinite(x[i]))
        x[i] = y[i]; /* modified to avoid potential inf */
    }
    x[0] = z[0] * (y[0] - x[n - 1]);
  }
  if (p->full_verify)
  ...
  for (l = 1; l <= loop; l++) {
    for (i = 1; i < n; i++) {
      for (k = 0; k < i; k++) {
        w[i] = b[k * n + i] * w[(i - k) - 1];
        if (!th_isfinite(w[i]))
          w[i] = b[k * n + i]; /* avoid potential inf */
      }
    }
    w[0] = b[k * n] * w[n - 1];
  }
...
e_fp state_fragment(loops_params *p) {
   ...
   *  7 CONTINUE
   *
   *	SG: Had to modify since outer loop only had to execute once if compiler
   *is smart enough
   */
  for (l = 1; l <= loop; l++) {
    reinit_vec(p, u, n + 6);
...
e_fp adi_integration(loops_params *p) {
  ...
  e_fp ret = FPCONST(0.0);
  e_fp *v =
      reinit_vec(p, p->v[3], 10); /* get a few radom values for the a[i] */
  e_fp a11 = v[0];
  e_fp a12 = v[1];
  e_fp a13 = v[2];
  e_fp a21 = v[3];
  ...
  for (l = 1; l <= loop; l++) {
    ...
    for (ky = 1; ky < n;) {
      ...
      __asm__ volatile("vsetvli %0, %0, e32, m1, ta, ma"
                       : "=r"(vl)
                       : "r"(n - ky));
      //         -2    -1     0   1     2    3    4    5
      ...
      __asm__ volatile(
          "vfmacc.vf      v30,  %0,  v4"
          :
          : "f"(-FPCONST(2.0))); //                                          u1p[2]-FPCONST(2.0)*u1p[1]
                                 //                                          ;
      __asm__ volatile(
          "vfadd.vv        v3, v30,  v3"); //                                                                    +u1p[0] ;
      __asm__ volatile("vmv.v.v        v30,  v4");
      __asm__ volatile("vfmacc.vf      v30,  %0, v24"
                       :
                       : "f"(a11)); // u1p[1]+a11*du11 ;
      __asm__ volatile("vfmacc.vf      v30,  %0, v25"
                       :
                       : "f"(a12)); //                +a12*du21 ;
      __asm__ volatile("vfmacc.vf      v30,  %0, v26"
                       :
                       : "f"(a13)); //                         +a13*du31 ;
      __asm__ volatile(
          "vfmacc.vf      v30,  %0,  v3"
      ...
      __asm__ volatile(
          "vfmacc.vf       v0,  %0,  v5"
          :
          : "f"(-FPCONST(2.0))); //                                          u1p[3]-FPCONST(2.0)*u1p[2]
                                 //                                          ;
      __asm__ volatile(
          "vfadd.vv        v0,  v0,  v4"); //                                                                    +u1p[1] ;
      __asm__ volatile("vmv.v.v        v31,  v5");
      __asm__ volatile("vfmacc.vf      v31,  %0, v27"
                       :
                       : "f"(a11)); // u1p[2]+a11*du12 ;
      __asm__ volatile("vfmacc.vf      v31,  %0, v28"
                       :
                       : "f"(a12)); //                +a12*du22 ;
      __asm__ volatile("vfmacc.vf      v31,  %0, v29"
                       :
                       : "f"(a13)); //                         +a13*du32 ;
      __asm__ volatile(
          "vfmacc.vf      v31,  %0,  v0"
      ...
      __asm__ volatile("vssseg2e32.v   v30, (%0), %1"
                       :
                       : "r"(&u1[ioutb + 1]), "r"(3 * sizeof(float)));

      __asm__ volatile("vmv.v.v        v30, v13");
      __asm__ volatile(
          "vfmacc.vf      v30,  %0, v12"
          :
          : "f"(-FPCONST(2.0))); //                                          u2p[2]-FPCONST(2.0)*u2p[1]
                                 //                                          ;
      __asm__ volatile(
          "vfadd.vv       v11, v30, v11"); //                                                                    +u2p[0] ;
      __asm__ volatile("vmv.v.v        v30, v12");
      __asm__ volatile("vfmacc.vf      v30,  %0, v24"
                       :
                       : "f"(a21)); // u2p[1]+a21*du11 ;
      __asm__ volatile("vfmacc.vf      v30,  %0, v25"
                       :
                       : "f"(a22)); //                +a22*du21 ;
      __asm__ volatile("vfmacc.vf      v30,  %0, v26"
      ...
      __asm__ volatile(
          "vfmacc.vf       v8,  %0, v13"
          :
          : "f"(-FPCONST(2.0))); //                                          u2p[3]-FPCONST(2.0)*u2p[2]
                                 //                                          ;
      __asm__ volatile(
          "vfadd.vv        v8,  v8, v12"); //                                                                    +u2p[1] ;
      __asm__ volatile("vmv.v.v        v31, v13");
      __asm__ volatile("vfmacc.vf      v31,  %0, v27"
                       :
                       : "f"(a21)); // u2p[2]+a21*du12 ;
      __asm__ volatile("vfmacc.vf      v31,  %0, v28"
                       :
                       : "f"(a22)); //                +a22*du22 ;
      __asm__ volatile("vfmacc.vf      v31,  %0, v29"
      ...
      __asm__ volatile("vssseg2e32.v   v30, (%0), %1"
                       :
                       : "r"(&u2[ioutb + 1]), "r"(3 * sizeof(float)));

      __asm__ volatile("vmv.v.v        v30, v21");
      __asm__ volatile(
          "vfmacc.vf      v30,  %0, v20"
          :
          : "f"(-FPCONST(2.0))); //                                          u3p[2]-FPCONST(2.0)*u3p[1]
                                 //                                          ;
      __asm__ volatile(
          "vfadd.vv       v19, v30, v19"); //                                                                    +u3p[0] ;
      __asm__ volatile("vmv.v.v        v30, v20");
      __asm__ volatile("vfmacc.vf      v30,  %0, v24"
                       :
                       : "f"(a31)); // u3p[1]+a31*du11 ;
      __asm__ volatile("vfmacc.vf      v30,  %0, v25"
                       :
                       : "f"(a32)); //                +a32*du21 ;
      __asm__ volatile("vfmacc.vf      v30,  %0, v26"
      ...
      __asm__ volatile(
          "vfmacc.vf      v16,  %0, v21"
          :
          : "f"(-FPCONST(2.0))); //                                          u3p[3]-FPCONST(2.0)*u3p[2]
                                 //                                          ;
      __asm__ volatile(
          "vfadd.vv       v16, v16, v20"); //                                                                    +u3p[1] ;
      __asm__ volatile("vmv.v.v        v31, v21");
      __asm__ volatile("vfmacc.vf      v31,  %0, v27"
                       :
                       : "f"(a31)); // u3p[2]+a31*du12 ;
      __asm__ volatile("vfmacc.vf      v31,  %0, v28"
                       :
                       : "f"(a32)); //                +a32*du22 ;
      __asm__ volatile("vfmacc.vf      v31,  %0, v29"
    ...
    /*   float u11 = u1p[1]+a11*du11+a12*du21+a13*du31 +
     * sig*(u1p[2]-FPCONST(2.0)*u1p[1]+u1p[0]); */
    /*   float u21 = u2p[1]+a21*du11+a22*du21+a23*du31 +
     * sig*(u2p[2]-FPCONST(2.0)*u2p[1]+u2p[0]); */
    /*   float u31 = u3p[1]+a31*du11+a32*du21+a33*du31 +
     * sig*(u3p[2]-FPCONST(2.0)*u3p[1]+u3p[0]); */

    /*   float u12 = u1p[2]+a11*du12+a12*du22+a13*du32 +
     * sig*(u1p[3]-FPCONST(2.0)*u1p[2]+u1p[1]); */
    /*   float u22 = u2p[2]+a21*du12+a22*du22+a23*du32 +
     * sig*(u2p[3]-FPCONST(2.0)*u2p[2]+u2p[1]); */
    /*   float u32 = u3p[2]+a31*du12+a32*du22+a33*du32 +
     * sig*(u3p[3]-FPCONST(2.0)*u3p[2]+u3p[1]); */

    /* printf("u1p %x %x %x %x %x %x %x %x\n", */
    /*        hex(u1p[-2]), hex(u1p[-1]), hex(u1p[0]), hex(u1p[1]), hex(u1p[2]),
     * hex(u1p[3]), hex(u1p[4]), hex(u1p[5])); */
    /* printf("u2p %x %x %x %x %x %x %x %x\n", */
    /*        hex(u2p[-2]), hex(u2p[-1]), hex(u2p[0]), hex(u2p[1]), hex(u2p[2]),
     * hex(u2p[3]), hex(u2p[4]), hex(u2p[5])); */
    /* printf("u3p %x %x %x %x %x %x %x %x\n", */
    /*        hex(u3p[-2]), hex(u3p[-1]), hex(u3p[0]), hex(u3p[1]), hex(u3p[2]),
     * hex(u3p[3]), hex(u3p[4]), hex(u3p[5])); */
    /* printf("du %x %x %x %x %x %x\n", hex(du11), hex(du21), hex(du31),
     * hex(du12), hex(du22), hex(du32)); */
    ...
    for (ky = 1; ky < n; ky++) {
      ...
      u1[(ioutb + 1)] = u1p[1] + a11 * du11 + a12 * du21 + a13 * du31 +
                        sig * (u1p[2] - FPCONST(2.0) * u1p[1] + u1p[0]);
      u2[(ioutb + 1)] = u2p[1] + a21 * du11 + a22 * du21 + a23 * du31 +
                        sig * (u2p[2] - FPCONST(2.0) * u2p[1] + u2p[0]);
      u3[(ioutb + 1)] = u3p[1] + a31 * du11 + a32 * du21 + a33 * du31 +
                        sig * (u3p[2] - FPCONST(2.0) * u3p[1] + u3p[0]);

      u1[(ioutb + 2)] = u1p[2] + a11 * du12 + a12 * du22 + a13 * du32 +
                        sig * (u1p[3] - FPCONST(2.0) * u1p[2] + u1p[1]);
      u2[(ioutb + 2)] = u2p[2] + a21 * du12 + a22 * du22 + a23 * du32 +
                        sig * (u2p[3] - FPCONST(2.0) * u2p[2] + u2p[1]);
      u3[(ioutb + 2)] = u3p[2] + a31 * du12 + a32 * du22 + a33 * du32 +
                        sig * (u3p[3] - FPCONST(2.0) * u3p[2] + u3p[1]);
    }
    ...
    /* } */
#endif
    ret += get_array_feedback(&u1[3 * n], 3 * n) +
           get_array_feedback(&u2[3 * n], 3 * n) +
           get_array_feedback(&u3[3 * n], 3 * n);
  }
  return ret;
}
...
e_fp integrate_predictors(loops_params *p) {
  ...
  for (l = 1; l <= loop; l++) {
    reinit_vec(p, dm,
               30); /* reinit to make sure each loop needs to be executed */
    for (i = 0; i < n; i++) {
      px[i * 13] = dm[28] * px[i * 13 + 12] + dm[27] * px[i * 13 + 11] +
                   dm[26] * px[i * 13 + 10] + dm[25] * px[i * 13 + 9] +
                   dm[24] * px[i * 13 + 8] + dm[23] * px[i * 13 + 7] +
                   dm[22] * px[i * 13 + 6] +
                   dm[0] * (px[i * 13 + 4] + px[i * 13 + 5]) + px[i * 13 + 2];
    }
    for (i = 0; i < n; i++)
      ret += px[i * 13];
...
e_fp difference_predictors(loops_params *p) {
  ...
  for (l = 1; l <= loop; l++) {
    reinit_vec(p, cx,
               n); /* initializers to make sure all loops need to be executed */
    for (i = 0; i < n; i++) {
      ar = cx[i];
      br = ar - px[14 * i + 4];
      px[14 * i + 4] = ar;
...
e_fp pic_2d(loops_params *p) {
  ...
  for (l = 1; l <= loop; l++) {
    ...
    for (ip = 0; ip < n;) {
      ...
      vuint64m4_t bcidxs64 = __riscv_vreinterpret_v_i64m4_u64m4(
          __riscv_vsext_vf2_i64m4(bcidxs, vl));
      ...
      vuint64m4_t yeidxs64 = __riscv_vreinterpret_v_i64m4_u64m4(
          __riscv_vsext_vf2_i64m4(yeidxs, vl));
      vint32m2_t zfidxs = __riscv_vadd_vx_i32m2(j2s, 32, vl);
      zfidxs = __riscv_vsll_vx_i32m2(j2s, 2, vl);
      vuint64m4_t zfidxs64 = __riscv_vreinterpret_v_i64m4_u64m4(
          __riscv_vsext_vf2_i64m4(zfidxs, vl));
      ...
      i2s = __riscv_vadd_vv_i32m2(i2s, __riscv_vreinterpret_v_u32m2_i32m2(es),
                                  vl);
      j2s = __riscv_vadd_vv_i32m2(j2s, __riscv_vreinterpret_v_u32m2_i32m2(fs),
                                  vl);
...
int is_2px(int x) { return (x & (x - 1)) == 0; }
e_fp pic_1d(loops_params *p) {
  ...
  if (!is_2px(n)) {
    th_exit(TH_ERROR,
            "Input for pic_1d in loops must be power of 2 [got %x].\n", n);
  }
...
e_fp casual(loops_params *p) {
  ...
  for (l = 1; l <= loop; l++) {
    ...
    /* vh, vf and vg must be modified at each loop, otherwise compiler can
     * optimize away */
    ...
    /* Save some value of the output so compiler does not optimize the compute
     * workload away */
...
e_fp monte_carlo(loops_params *p) {
  ...
  for (l = 1; l <= loop; l++) {
    int i1, j2, j4, j5;
    i1 = m = 1;
    /* inits to make sure new inputs each loop iteration so compiler does not
     * optimize the work away */
  ...
  label410:
    ...
    /* return value should have an update output for wach loop iteration so it
...
e_fp implicit(loops_params *p) {
  ...
  for (l = 1; l <= loop; l++) {
    ...
    e_fp xnm = start_xnm;
    e_fp e6 = start_e6;
    /* add an epsilon every iteration otherwise compiler only needs to execute
     * last iteration */
  ...
  for (l = 1; l <= loop; l++) {
    ...
    for (k = 1; k < kn; k++) {
      for (j = 1; j < jn; j++) {
        ...
        if (!th_isfinite(za[cur]))
          za[cur] = FPCONST(1.0);
        if (!th_isfinite(zb[cur]))
          zb[cur] = FPCONST(1.0);
      }
    }
    for (k = 1; k < kn; k++) {
    ...
    for (k = 1; k < kn; k++) {
      for (j = 1; j < jn; j++) {
        ...
        if (!th_isfinite(zr[cur]))
          zr[cur] = FPCONST(1.0);
        if (!th_isfinite(zz[cur]))
          zz[cur] = FPCONST(1.0);
      }
    }
...
e_fp ordinate_transport(loops_params *p) {
  ...
  dk = xx[2];
  for (l = 1; l <= loop; l++) {
    reinit_vec(p, xx, 1); /* start a new input each outer loop iteration
                             otherwise only last iteration will be executed */
    for (k = 0; k < n - 1; k++) {
      di = y[k] - g[k] / (xx[k] + dk);
      dn = 0.2;
      if (di) {
    ...
    ret +=
        xx[n - 1]; /* return an output value dependent on each loop iteration */
#if (BMDEBUG && DEBUG_ACCURATE_BITS)
    th_printf("\nord %d:", debug_counter++);
    th_print_fp(ret);
#endif
...
e_fp matmul(loops_params *p) {
   ...
   * 21 CONTINUE
   */
  /*SG: Add dependency on outer loop to CX so compiler cannot optimize it away
   * to a multiply on the final result. */
  for (l = 1; l <= loop; l++) {
#if USE_RVV
    for (j = 0; j < n; j++) {
      for (i = 0; i < 25;) {
...
e_fp planckian(loops_params *p) {
  ...
  for (l = 1; l <= loop; l++) {
    reinit_vec(p, v,
               n); /* must reinit otherwise compiler will only exec last loop */
    for (k = 0; k < n; k++) {
      y[k] = u[k] / v[k];
      w[k] = x[k] / (th_exp(y[k]) - FPCONST(1.0));
    }
    ret += get_array_feedback(
        w, n); /* return a value that depends on each compute in the loop */
  }
  return ret;
}
...
e_fp hydro_2d_implicit(loops_params *p) {
  ...
  ret = get_array_feedback(
      za, n); /* return a value that depends on each compute in the loop */
  return ret;
}
...
e_fp firstmin(loops_params *p) {
  ...
  for (l = 1; l <= loop; l++) {
    reinit_vec(
        p, x,
        n); /* must reinit otherwise compiler can execute only the last loop */
