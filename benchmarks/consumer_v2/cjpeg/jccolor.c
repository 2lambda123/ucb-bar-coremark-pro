...
rgb_ycc_start(j_compress_ptr cinfo) {
  ...
  cconvert->rgb_ycc_tab = rgb_ycc_tab = (e_s32 *)(*cinfo->mem->alloc_small)(
      (j_common_ptr)cinfo, JPOOL_IMAGE, (TABLE_SIZE * SIZEOF(e_s32)));
  ...
  while (--num_rows >= 0) {
    ...
    for (col = 0; col < num_cols;) {
      ...
      __asm__ volatile("vsetvli %0, %1, e8, m1, ta, ma"
                       : "=r"(vl)
                       : "r"(num_cols - col));
      __asm__ volatile("vlseg3e8.v v0, (%0)"
                       :
                       : "r"(&inptr[col * RGB_PIXELSIZE]));
