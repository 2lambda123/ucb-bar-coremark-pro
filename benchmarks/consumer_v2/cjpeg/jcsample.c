...
fullsize_downsample(j_compress_ptr cinfo, jpeg_component_info *compptr,
                    JSAMPARRAY input_data, JSAMPARRAY output_data) {
  /* Copy the data */
  cjcopy_sample_rows(input_data, 0, output_data, 0, cinfo->max_v_samp_factor,
                     cinfo->image_width);
  /* Edge-expand */
...
#if USE_RVV
    size_t vl;
    __asm__ volatile("vsetvli %0, %1, e16, m8, ta, ma"
                     : "=r"(vl)
                     : "r"(output_cols));
    __asm__ volatile("vmv.v.x v0, %0" : : "r"((uint16_t)0x0201));
    for (outcol = 0; outcol < output_cols;) {
      __asm__ volatile("vsetvli %0, %1, e8, m4, ta, ma"
                       : "=r"(vl)
                       : "r"(output_cols - outcol));
